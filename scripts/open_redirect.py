...
def find_open_redirect_vulnerabilities(urls):
    console = Console()
    config = load_config()
    ...
    for url in urls:
        prompt = f"Check if this URL is vulnerable to open redirect: {url}"
        try:
            ...
            if "vulnerable" in result.lower():
                open_redirect_vulnerabilities.append(url)
...
def run():
    console = Console()
    console.print("[title]🔍 Running open redirect vulnerability scan...[/title]")
    ...
    open_redirect_vulnerabilities = find_open_redirect_vulnerabilities(urls)

    with open(OUTPUT_DIR / "open_redirect_vulnerabilities.txt", "w") as f:
        for url in open_redirect_vulnerabilities:
            f.write(url + '\n')

    console.print("[success]Open redirect vulnerability scan completed! 🎉[/success]")
