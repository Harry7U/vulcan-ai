...
def find_command_injection_vulnerabilities(urls):
    console = Console()
    config = load_config()
    ...
    for url in urls:
        prompt = f"Check if this URL is vulnerable to command injection: {url}"
        try:
            ...
            if "vulnerable" in result.lower():
                command_injection_vulnerabilities.append(url)
...
def run():
    console = Console()
    console.print("[title]🔍 Running command injection vulnerability scan...[/title]")
    ...
    command_injection_vulnerabilities = find_command_injection_vulnerabilities(urls)

    with open(OUTPUT_DIR / "command_injection_vulnerabilities.txt", "w") as f:
        for url in command_injection_vulnerabilities:
            f.write(url + '\n')

    console.print("[success]Command injection vulnerability scan completed! 🎉[/success]")
